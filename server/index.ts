...
io.on("connection", (socket: Socket) => {
  ...
  socket.on(SocketEvents.CREATE_ROOM, (data: { playerName: string }, callback) => {
    try {
      console.log("[v0] CREATE_ROOM event received:", { playerName: data.playerName, socketId: socket.id })
      ...
      console.log(`[v0] Room ${roomId} created by ${data.playerName}`)
      ...
      console.error("[v0] Error in CREATE_ROOM:", error)
  ...
  socket.on(SocketEvents.JOIN_ROOM, (data: { roomId: string; playerName: string }, callback) => {
    try {
      console.log("[v0] JOIN_ROOM event received:", {
        roomId: data.roomId,
      ...
      if (!room) {
        console.log(`[v0] Room ${roomId} not found`)
        callback({ success: false, error: "Room not found" })
        return
      }

      if (room.players.length >= 2) {
        console.log(`[v0] Room ${roomId} is full`)
        callback({ success: false, error: "Room is full" })
        return
      }

      if (room.gameStarted || room.finished) {
        console.log(`[v0] Game in room ${roomId} already in progress`)
      ...
      console.log(`[v0] Room ${roomId}: ${data.playerName} joined (Total: ${room.players.length}/2)`)
      ...
      if (room.players.length === 2) {
        ...
        console.log(`[v0] Room ${roomId}: Game is starting!`)
        setTimeout(() => {
      ...
      console.error("[v0] Error in JOIN_ROOM:", error)
  ...
  socket.on(SocketEvents.MAKE_CHOICE, (data: { choice: "rock" | "paper" | "scissors" }, callback) => {
    try {
      ...
      if (!room) {
        console.log("[v0] Room not found for MAKE_CHOICE")
        callback({ success: false, error: "Room not found" })
        return
      }
      ...
      if (player) {
        ...
        console.log(`[v0] Room ${roomId}: ${socket.data.playerName} chose ${data.choice}`)
      }
      ...
      if (room.players.every((p) => p.choice !== null)) {
        console.log(`[v0] Room ${roomId}: Both players chose, processing round...`)
        processRound(io, roomId, room)
      }
    } catch (error) {
      console.error("[v0] Error in MAKE_CHOICE:", error)
  ...
  socket.on(SocketEvents.RESET_ROUND, () => {
    try {
      ...
      room.currentRound++
      console.log(`[v0] Room ${roomId}: Moving to round ${room.currentRound}`)

      const player1Wins = room.scores[room.players[0].id] || 0
      const player2Wins = room.scores[room.players[1].id] || 0

      console.log(`[v0] Room ${roomId}: Scores after round - P1: ${player1Wins}, P2: ${player2Wins}`)

      if (player1Wins >= 2 || player2Wins >= 2) {
        room.finished = true
        room.gameStarted = false
        const winner = player1Wins >= 2 ? room.players[0] : room.players[1]
        console.log(`[v0] Room ${roomId}: Game finished! Winner: ${winner.name}`)
        ...
        console.log(`[v0] Room ${roomId}: Starting next round ${room.currentRound}`)
        io.to(roomId).emit(SocketEvents.ROUND_STARTED, {
          round: room.currentRound,
          room,
        })
      ...
      console.error("[v0] Error in RESET_ROUND:", error)
    }
  })
  ...
  socket.on(SocketEvents.PLAY_AGAIN, () => {
    try {
      ...
      if (!room) {
        console.log("[v0] Room not found for PLAY_AGAIN")
        return
      }

      console.log(`[v0] Room ${roomId}: ${socket.data.playerName} wants to play again`)
      ...
      console.log(`[v0] Room ${roomId}: ${requests.size}/${room.players.length} players want to play again`)

      // If both players want to play again, restart the game
      if (requests.size === room.players.length && room.players.length === 2) {
        console.log(`[v0] Room ${roomId}: Restarting game!`)
      ...
      console.error("[v0] Error in PLAY_AGAIN:", error)
    }
  })
  ...
  socket.on(SocketEvents.LEAVE_ROOM, () => {
    try {
      ...
      console.log(`[v0] Room ${roomId}: ${socket.data.playerName} is leaving`)
      ...
      if (room.players.length === 0) {
        rooms.delete(roomId)
        playAgainRequests.delete(roomId)
        console.log(`[v0] Room ${roomId} deleted (empty)`)
      }
    } catch (error) {
      console.error("[v0] Error in LEAVE_ROOM:", error)
    }
  })
  ...
  socket.on("disconnect", () => {
    const roomId = socket.data.roomId
    if (!roomId) {
      console.log("[v0] Socket disconnected (no room):", socket.id)
      return
    }
    ...
    if (player) {
      ...
      console.log(`[v0] Room ${roomId}: ${socket.data.playerName} disconnected`)
    }
    ...
    setTimeout(() => {
      if (!room.players.some((p) => p.connected)) {
        rooms.delete(roomId)
        playAgainRequests.delete(roomId)
        console.log(`[v0] Room ${roomId} deleted due to inactivity`)
      }
    }, 60000)
  })
})
