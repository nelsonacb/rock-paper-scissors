...
export function GameView({ socket, roomCode, isCreator, onGameEnd }: GameViewProps) {
  ...
  useEffect(() => {
    ...
    socket.on(SocketEvents.ROUND_STARTED, (data: { room: GameRoom; round: number }) => {
    ...
    socket.on(SocketEvents.PLAYER_JOINED, (data: { room: GameRoom; players: any[] }) => {
    ...
    socket.on(
      ...
      (data: { result: RoundResult; scores: { [key: string]: number }; room: GameRoom }) => {
    ...
    socket.on(SocketEvents.GAME_ENDED, (data: { room: GameRoom; winner: any; scores: { [key: string]: number } }) => {
      ...
      if (data.winner.id === myPlayerId) {
        notifications.success("You Won!", `Congratulations, ${data.winner.name}!`)
      } else {
        notifications.error("Game Over", `${data.winner.name} won the match`)
      }
    })

    socket.on(SocketEvents.PLAYER_DISCONNECTED, (data: { room: GameRoom; players: any[] }) => {
      console.log("Player disconnected:", data)
      setRoom(data.room)
      notifications.warning("Player Disconnected", "Waiting for reconnection...")
    })

    socket.on(
      SocketEvents.WAITING_FOR_REMATCH,
      (data: { playersReady: number; totalPlayers: number; room: GameRoom }) => {
        console.log("Waiting for rematch:", data)
        setRematchPlayersReady(data.playersReady)
        if (data.playersReady < data.totalPlayers) {
          notifications.info("Rematch", `Waiting for opponent (${data.playersReady}/${data.totalPlayers})`)
        }
      },
    )
    ...
    socket.on(SocketEvents.PLAYER_LEFT, (data: { playerName: string; room: GameRoom }) => {
      console.log("Player left:", data)
      notifications.warning("Player Left", `${data.playerName} has left the game`)
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 md:py-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-2xl md:text-3xl font-mono font-bold text-primary">{roomCode}</p>
          </div>
          <RoundInfo
        ...
        <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-8">
          ...
          <div>
            <PlayerStatus player={opponent} isYou={false} score={scores[opponent.id]} choice={opponent.choice} />
          </div>
        </div>
        ...
        {room.gameStarted && !gameFinished && (
          <div className="card-base mb-8">
            <GameChoices
              ...
              disabled={!room.gameStarted || gameFinished || selectedChoice !== null}
        ...
        {roundResult && nextRoundReady && (
          <div className="card-base mb-8 text-center space-y-6 animate-in fade-in duration-500">
            <h3 className="text-2xl md:text-3xl font-bold">Round {roundResult.round} Result</h3>
            ...
            {scores[player1.id] >= 2 || scores[player2.id] >= 2 ? (
              <div className="pt-4">
                <button onClick={() => setGameFinished(true)} className="btn-primary">
                  See Final Results
                </button>
              </div>
            ) : (
        ...
        {!room.gameStarted && room.players.length < 2 && (
          <div className="card-base text-center py-12">
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">Share the room code: {roomCode}</p>
