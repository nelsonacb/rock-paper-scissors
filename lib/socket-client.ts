...
export function initializeSocketClient(): Socket {
  if (socket && socket.connected) {
    console.log("[v0] Socket already connected:", socket.id)
    return socket
  }
  ...
  console.log("[v0] Initializing Socket.io client with URL:", socketUrl)
  ...
  socket.on("connect", () => {
    console.log("[v0] Socket.io Client Connected with ID:", socket!.id)
  })
  ...
  socket.on("disconnect", () => {
    console.log("[v0] Socket.io Client Disconnected")
  })
...
export function disconnectSocket() {
  if (socket) {
    console.log("[v0] Disconnecting socket:", socket.id)
    socket.disconnect()
    socket = null
  }
}
...
// Event handlers
export function onRoomCreated(callback: (data: { roomId: string; players: any[] }) => void) {
  const s = getSocket()
