...
export function processRound(io: any, roomId: string, room: GameRoom) {
  ...
  console.log(
    `[v0] Room ${roomId} Round ${room.currentRound}: ${
      winner === "draw" ? "Draw - replay this round" : winner === 1 ? player1.name : player2.name
    } wins. Scores: ${room.scores[player1.id] || 0} - ${room.scores[player2.id] || 0}`,
  )
